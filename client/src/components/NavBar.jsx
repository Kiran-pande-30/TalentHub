          ...
          {['business', 'freelancers', 'works'].map((item) => (
            ...
            >
              <NavLink
                to={`/${item}`}
                className={({ isActive }) =>
                  isActive ? 'font-semibold text-[#4169e1]' : ''
                }
              >
            ...
            <>
              <motion.div variants={linkVariants} whileHover="hover">
                <NavLink
                  to="/user/dashboard"
                  className={({ isActive }) =>
                    isActive ? 'font-semibold text-[#40a2c9]' : ''
                  }
                >
                  Profile
                </NavLink>
              ...
              <motion.button
                whileHover={{
                  scale: 1.05,
                  backgroundColor: '#40A2C9',
                  color: 'white',
                }}
                whileTap={{ scale: 0.95 }}
                onClick={handleLogout}
                className="rounded-lg border-2 border-[#40a2c9] px-4 py-2 text-[#40a2c9] transition-colors"
              >
                Logout
              </motion.button>
            </>
            ...
            <div className="flex items-center justify-between px-6 py-4 shadow-md">
              ...
              >
                <Link to="/" onClick={() => setIsVisible(false)}>
                  <span>TalentHub</span>
                  <span className="text-[#40a2c9]">.</span>
                </Link>
              </motion.div>
              <motion.button
                    ...
                    <NavLink
                      ...
                      className={({ isActive }) =>
                        `w-full border p-3 transition-colors ${
                          isActive
                            ? 'bg-[#40A2C9]/10 font-semibold text-[#40a2c9]'
                            : 'hover:bg-gray-50'
                        }`
                      }
                    >
                ...
                {user ? (
                  <>
                    ...
                    <motion.button
                      ...
                      whileTap={{ scale: 0.98 }}
                      onClick={handleLogout}
                      className="mt-4 rounded-lg border-2 border-[#40a2c9] px-4 py-2 text-[#40a2c9] transition-colors"
                    >
                      Logout
                    </motion.button>
                  </>
